...
BOOST_AUTO_TEST_SUITE(creating_and_object_of_class_lines)
	...
	BOOST_AUTO_TEST_CASE(can_move_the_string_where_movable_string_to_become_empty)
	{
...
BOOST_AUTO_TEST_SUITE(assignment_operator)
	...
	BOOST_AUTO_TEST_CASE(can_assign_a_string_value_of_displacement)
	{
		...
		str = std::move(CMyString("bottom - down"));
...
BOOST_AUTO_TEST_SUITE(greater_operator)
	...
	{
		BOOST_CHECK((CMyString("abz") > CMyString("ab", 2)));
	}
